
const MILKCHOC_BANANA_GANACHE = {
	id: "MILKCHOC_BANANA_GANACHE",
	title: "Milk Chocolate Ganache with Banana",
	label: "chocolate banana ganache",
	ingredients: [
		{ qty: 130, unit: "gram", name: "ghirardelli-milk-chocolate" },
		{ qty: 90, unit: "gram", name: "banana" },
		{ qty: 15, unit: "milliliter", name: "lucerne-heavy-cream" },
		{ qty: 20, unit: "gram", name: "honey" },
		{ qty: 1, unit: "gram", name: "kosher-salt", note: "pinch" },
		{ qty: 12, unit: "piece", name: "dried-banana" },
	],
	directions: [
		"If using to fill tarts, ready the baked shells to be filled and set to the side",
		"Chop the chocolate into small pieces if using callets, otherwise put it into heat safe bowl and warm over bain-marie",
		"Mash the banana, add it to a saucepan with the cream and honey and bring to the boil",
		"Pour the cream over the chocolate and stir with a spatula in the middle of the bowl and always stirring the in the same direction until thoroughly mixed",
		"Add the salt and mix the ganache for a minute or two with a hand-held stick blender for the best result. The consistency should be smooth and glossy",
		"Pour immediately into tart shells and put in fridge for 1hr to set",
		"Garnish with a piece of dried banana (optional)"
	],
	notes: [

	]
}

export default MILKCHOC_BANANA_GANACHE;