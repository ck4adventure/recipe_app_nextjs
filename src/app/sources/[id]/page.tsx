...
export default async function Page ({ params }: { params: { id: number }}) {
	...
	const { title, name, is_profi, source_url, author_id, source_type } = sourceInfo;
  
	return (
		<div className="flex flex-col items-center">
			<div className="text-2xl font-bold">{title}</div>
			{/* TODO add cute logo for source type */}
			<div className="text-sm">By:<Link href={`/authors/${author_id}`}> {name}, {is_profi? 'Professional' : 'Amateur'} Chef </Link></div>
