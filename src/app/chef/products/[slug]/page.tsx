...
export default async function Page({ params }: { params: { slug: string } }) {
	...
	// NEXT STEP TRANSFORM COMPONENTS
	// let componentsObject = {
	// 	"base": [],
	// 	"filling": [],
	// 	"topping": []
	// };
	// components.forEach((result: any) => {
	// 	const c: string = result.component;
	// 	componentsObject[c] = result.recipes;
	// });

	// CONSIDER CARD LAYOUT
		...
		<div data-cy='product-item-page' className='flex flex-col items-center'>
			...
			<div className="max-w-[850px]">
				{/* <div data-cy='product-components-section' className="m-4">
					...
					<ul className="">
						{product.components && product.components.map((recipe: any) => (
							<li data-cy='product-recipe' key={product.title} className="flex my-1">
				...
				<div data-cy='product-method-section' className="m-4">
					...
					<ol className="list-decimal">
						{product.steps && product.steps.map((step: any, i: number) => (
				...
				{product.notes && product.notes.length > 0 && (
					<div data-cy='product-notes-section' className="m-4">
						...
						<ul>
							{product.notes && product.notes.map((note: string, i: number) => (
								<li key={i}>{note}</li>
							))}
						</ul>
					</div>
